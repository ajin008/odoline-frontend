...
export function FinancialSnapshot() {
  ...
  return (
    <div className="rounded-2xl border border-line bg-card p-4 space-y-3.5 select-none font-sans transition-all hover:border-accent/40">
      ...
      {/* 2. Compact 3-Column Core Metrics Grid */}
      <div className="grid grid-cols-3 gap-2.5">
        {/* Metric 1: Total Invested (Purchase + Refurbish) */}
        <div className="rounded-xl border border-line/50 bg-inset p-2.5 space-y-1">
          <div className="flex items-center gap-1 text-[10px] font-medium text-ink-muted">
            <Vault className="h-3 w-3 text-blue-500 shrink-0" />
            <span className="truncate">Invested Capital</span>
          </div>
          {isLoading ? (
            <div className="h-5 w-16 animate-pulse rounded bg-line/20" />
          ) : (
            <p
              className="font-heading text-sm sm:text-base font-bold text-ink tracking-tight truncate"
        ...
        <div className="rounded-xl border border-line/50 bg-inset p-2.5 space-y-1">
          <div className="flex items-center gap-1 text-[10px] font-medium text-ink-muted">
            <Coins className="h-3 w-3 text-purple-500 shrink-0" />
        ...
        <div className="rounded-xl border border-emerald-500/30 bg-emerald-500/5 p-2.5 space-y-1">
          <div className="flex items-center justify-between text-[10px] font-medium text-emerald-700">
            ...
            {roiPct > 0 && (
              <span className="inline-flex items-center text-[9px] font-bold text-emerald-600 font-mono">
          ...
          {isLoading ? (
            <div className="h-5 w-16 animate-pulse rounded bg-emerald-500/20" />
          ) : (
            <p
              className="font-heading text-sm sm:text-base font-bold text-emerald-600 tracking-tight truncate"
              title={formatFullCurrency(profit)}
            >
              +{formatCompactCurrency(profit)}
            </p>
          )}
          <span className="text-[9px] text-emerald-600/80 block truncate font-mono font-medium">
            Expected Margin
          </span>
        </div>
      </div>
      ...
      <div className="rounded-xl border border-line/60 bg-inset/50 p-2.5 space-y-1.5">
        ...
        <div className="h-2 w-full rounded-full bg-line/30 overflow-hidden flex">
          <div
