"use client";

import { useState } from "react";
import {
  useLeadFollowUps,
  useScheduleFollowUp,
  useUpdateFollowUp,
} from "../hooks/use-lead-follow-ups";
import type {
  FollowUp,
  NextFollowUpInfo,
  FollowUpDueState,
} from "../types/lead-types";
import { DatePicker } from "@/src/components/ui/date-picker";
import {
  CalendarCheck,
  Clock,
  CheckCircle2,
  XCircle,
  Calendar,
  Plus,
  Loader2,
  X,
  RotateCcw,
} from "lucide-react";

interface LeadFollowUpsProps {
  leadId: string;
  nextFollowUp?: NextFollowUpInfo | null;
}

const DUE_STATE_CONFIG: Record<
  FollowUpDueState,
  { label: string; bgClass: string; textClass: string; borderClass: string }
> = {
  overdue: {
    label: "Overdue",
    bgClass: "bg-red-500/10",
    textClass: "text-red-500",
    borderClass: "border-red-500/20",
  },
  due_today: {
    label: "Due Today",
    bgClass: "bg-amber-500/10",
    textClass: "text-amber-500",
    borderClass: "border-amber-500/20",
  },
  upcoming: {
    label: "Upcoming",
    bgClass: "bg-blue-500/10",
    textClass: "text-blue-500",
    borderClass: "border-blue-500/20",
  },
  none: {
    label: "No Reminder",
    bgClass: "bg-inset",
    textClass: "text-ink-subtle",
    borderClass: "border-line",
  },
};

function getTodayISTDateString(): string {
  const now = new Date();
  const istOffsetMs = 5.5 * 60 * 60 * 1000;
  const istDate = new Date(now.getTime() + istOffsetMs);
  const year = istDate.getUTCFullYear();
  const month = String(istDate.getUTCMonth() + 1).padStart(2, "0");
  const day = String(istDate.getUTCDate()).padStart(2, "0");
  return `${year}-${month}-${day}`;
}

export function LeadFollowUps({ leadId, nextFollowUp }: LeadFollowUpsProps) {
  const { data: followUps, isLoading, isError } = useLeadFollowUps(leadId);
  const scheduleMutation = useScheduleFollowUp(leadId);
  const updateMutation = useUpdateFollowUp(leadId);

  // Modals state
  const [isScheduleOpen, setIsScheduleOpen] = useState(false);
  const [scheduleDate, setScheduleDate] = useState(getTodayISTDateString());

  const [activeDoneFu, setActiveDoneFu] = useState<FollowUp | null>(null);
  const [outcomeNote, setOutcomeNote] = useState("");
  const [andScheduleNext, setAndScheduleNext] = useState(false);
  const [nextDate, setNextDate] = useState(getTodayISTDateString());

  const [activeRescheduleFu, setActiveRescheduleFu] = useState<FollowUp | null>(
    null
  );
  const [rescheduleDate, setRescheduleDate] = useState(getTodayISTDateString());

  const handleScheduleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    const isoDateTime = new Date(`${scheduleDate}T10:00:00.000Z`).toISOString();

    scheduleMutation.mutate(
      { due_at: isoDateTime },
      {
        onSuccess: () => {
          setIsScheduleOpen(false);
        },
      }
    );
  };

  const handleMarkDoneSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!activeDoneFu) return;

    updateMutation.mutate(
      {
        fuId: activeDoneFu.id,
        payload: {
          status: "done",
          outcome_note: outcomeNote.trim() || undefined,
        },
      },
      {
        onSuccess: () => {
          setActiveDoneFu(null);
          setOutcomeNote("");

          if (andScheduleNext) {
            const isoDateTime = new Date(
              `${nextDate}T10:00:00.000Z`
            ).toISOString();
            scheduleMutation.mutate({ due_at: isoDateTime });
          }
        },
      }
    );
  };

  const handleRescheduleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!activeRescheduleFu) return;

    const isoDateTime = new Date(
      `${rescheduleDate}T10:00:00.000Z`
    ).toISOString();

    updateMutation.mutate(
      {
        fuId: activeRescheduleFu.id,
        payload: {
          due_at: isoDateTime,
        },
      },
      {
        onSuccess: () => {
          setActiveRescheduleFu(null);
        },
      }
    );
  };

  const handleCancelFollowUp = (fu: FollowUp) => {
    updateMutation.mutate({
      fuId: fu.id,
      payload: {
        status: "cancelled",
      },
    });
  };

  const openFollowUps = followUps?.filter((f) => f.status === "open") || [];
  const activeNextFollowUp = openFollowUps[0];
  const dueStateConfig =
    DUE_STATE_CONFIG[nextFollowUp?.due_state || "none"] ||
    DUE_STATE_CONFIG.none;

  return (
    <div className="rounded-xl border border-line bg-card p-5 space-y-4 font-sans select-none">
      {/* Header & Schedule Button */}
      <div className="flex items-center justify-between gap-2 border-b border-line/60 pb-3">
        <h3 className="text-xs font-semibold uppercase tracking-wider text-ink-muted flex items-center gap-2">
          <Clock className="h-4 w-4 text-accent" />
          Follow-up Schedule
        </h3>

        <button
          type="button"
          onClick={() => setIsScheduleOpen(true)}
          className="flex items-center gap-1.5 rounded-lg bg-accent px-3 py-1.5 text-xs font-semibold text-inverse shadow-xs hover:opacity-90 transition-opacity cursor-pointer"
        >
          <Plus className="h-3.5 w-3.5" />
          <span>Schedule</span>
        </button>
      </div>

      {/* Prominent Next Follow-up Banner */}
      {nextFollowUp && activeNextFollowUp ? (
        <div
          className={`rounded-lg border p-4 space-y-3 ${dueStateConfig.bgClass} ${dueStateConfig.borderClass}`}
        >
          <div className="flex items-center justify-between gap-2">
            <span
              className={`inline-flex items-center rounded-md border px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider ${dueStateConfig.bgClass} ${dueStateConfig.textClass} ${dueStateConfig.borderClass}`}
            >
              {dueStateConfig.label}
            </span>

            <div className="flex items-center gap-1.5">
              <button
                type="button"
                onClick={() => {
                  setActiveDoneFu(activeNextFollowUp);
                  setOutcomeNote("");
                  setAndScheduleNext(false);
                }}
                className="flex items-center gap-1 rounded-md bg-emerald-500/20 text-emerald-600 dark:text-emerald-400 hover:bg-emerald-500/30 px-2.5 py-1 text-xs font-semibold transition-colors cursor-pointer"
              >
                <CheckCircle2 className="h-3.5 w-3.5" />
                <span>Mark Done</span>
              </button>

              <button
                type="button"
                onClick={() => {
                  setActiveRescheduleFu(activeNextFollowUp);
                  setRescheduleDate(getTodayISTDateString());
                }}
                className="p-1 rounded-md text-ink-subtle hover:text-ink hover:bg-card/60 transition-colors cursor-pointer"
                title="Reschedule"
              >
                <RotateCcw className="h-3.5 w-3.5" />
              </button>

              <button
                type="button"
                onClick={() => handleCancelFollowUp(activeNextFollowUp)}
                className="p-1 rounded-md text-ink-subtle hover:text-danger hover:bg-card/60 transition-colors cursor-pointer"
                title="Cancel"
              >
                <XCircle className="h-3.5 w-3.5" />
              </button>
            </div>
          </div>

          <div className="flex items-center gap-2 text-xs font-semibold text-ink">
            <Calendar className="h-4 w-4 text-accent shrink-0" />
            <span>
              Due:{" "}
              {new Date(nextFollowUp.due_at).toLocaleDateString("en-IN", {
                weekday: "short",
                day: "numeric",
                month: "short",
                year: "numeric",
              })}
            </span>
          </div>
        </div>
      ) : (
        <div className="rounded-lg border border-dashed border-line bg-inset/40 p-4 text-center text-xs text-ink-subtle space-y-1">
          <p className="font-semibold text-ink">No Open Follow-up</p>
          <p className="text-[11px]">
            Schedule a reminder date to keep this lead active and tracked.
          </p>
        </div>
      )}

      {/* History & Closed Follow-ups */}
      {isLoading ? (
        <div className="py-4 flex items-center justify-center gap-2 text-xs text-ink-subtle">
          <Loader2 className="h-4 w-4 animate-spin text-accent" />
          Loading follow-ups…
        </div>
      ) : isError ? (
        <p className="text-xs text-danger text-center py-2">
          Could not load follow-up list.
        </p>
      ) : followUps && followUps.length > 0 ? (
        <div className="space-y-2 pt-2 border-t border-line/50">
          <span className="text-[11px] font-semibold text-ink-subtle uppercase tracking-wider">
            All Reminders ({followUps.length})
          </span>

          <div className="space-y-2 max-h-[220px] overflow-y-auto pr-1">
            {followUps.map((fu) => {
              const dueStr = new Date(fu.due_at).toLocaleDateString("en-IN", {
                day: "numeric",
                month: "short",
                year: "numeric",
              });

              if (fu.status === "open") {
                return (
                  <div
                    key={fu.id}
                    className="flex items-center justify-between gap-2 p-2.5 rounded-lg border border-line bg-inset/40 text-xs"
                  >
                    <div className="flex items-center gap-2">
                      <Clock className="h-3.5 w-3.5 text-accent shrink-0" />
                      <span className="font-medium text-ink">{dueStr}</span>
                      <span className="text-[10px] font-bold text-amber-500 uppercase">
                        (Open)
                      </span>
                    </div>

                    <div className="flex items-center gap-1">
                      <button
                        type="button"
                        onClick={() => {
                          setActiveDoneFu(fu);
                          setOutcomeNote("");
                          setAndScheduleNext(false);
                        }}
                        className="p-1 rounded text-emerald-500 hover:bg-emerald-500/10 transition-colors cursor-pointer"
                        title="Mark Done"
                      >
                        <CheckCircle2 className="h-3.5 w-3.5" />
                      </button>
                      <button
                        type="button"
                        onClick={() => handleCancelFollowUp(fu)}
                        className="p-1 rounded text-ink-subtle hover:text-danger hover:bg-card/60 transition-colors cursor-pointer"
                        title="Cancel"
                      >
                        <XCircle className="h-3.5 w-3.5" />
                      </button>
                    </div>
                  </div>
                );
              }

              return (
                <div
                  key={fu.id}
                  className="space-y-1 p-2.5 rounded-lg border border-line/50 bg-card text-xs opacity-75"
                >
                  <div className="flex items-center justify-between text-ink-subtle">
                    <span className="flex items-center gap-1.5">
                      {fu.status === "done" ? (
                        <CheckCircle2 className="h-3.5 w-3.5 text-emerald-500 shrink-0" />
                      ) : (
                        <XCircle className="h-3.5 w-3.5 text-danger shrink-0" />
                      )}
                      <span className="font-medium text-ink">{dueStr}</span>
                    </span>
                    <span className="text-[10px] uppercase font-bold tracking-wider">
                      {fu.status}
                    </span>
                  </div>

                  {fu.outcome_note && (
                    <p className="text-[11px] text-ink italic pl-5">
                      &ldquo;{fu.outcome_note}&rdquo;
                    </p>
                  )}
                </div>
              );
            })}
          </div>
        </div>
      ) : null}

      {/* Modal 1: Schedule New Follow-up */}
      {isScheduleOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-xs p-4">
          <div className="w-full max-w-sm rounded-xl border border-line bg-card p-5 shadow-xl space-y-4">
            <div className="flex items-center justify-between border-b border-line pb-3">
              <h4 className="text-sm font-bold text-ink flex items-center gap-2">
                <CalendarCheck className="h-4 w-4 text-accent" />
                Schedule Follow-up
              </h4>
              <button
                type="button"
                onClick={() => setIsScheduleOpen(false)}
                className="text-ink-subtle hover:text-ink transition-colors p-1"
              >
                <X className="h-4 w-4" />
              </button>
            </div>

            <form onSubmit={handleScheduleSubmit} className="space-y-4">
              <div className="space-y-1.5">
                <label className="text-xs font-medium text-ink-muted">
                  Follow-up Date (IST)
                </label>
                <div>
                  <DatePicker
                    value={scheduleDate}
                    onChange={(d) => setScheduleDate(d)}
                    align="left"
                  />
                </div>
              </div>

              <div className="flex items-center justify-end gap-2 pt-2">
                <button
                  type="button"
                  onClick={() => setIsScheduleOpen(false)}
                  className="rounded-lg border border-line px-3.5 py-2 text-xs font-medium text-ink hover:bg-hover transition-colors"
                >
                  Cancel
                </button>
                <button
                  type="submit"
                  disabled={scheduleMutation.isPending}
                  className="flex items-center gap-1.5 rounded-lg bg-accent px-4 py-2 text-xs font-semibold text-inverse shadow-xs hover:opacity-90 transition-opacity disabled:opacity-50"
                >
                  {scheduleMutation.isPending && (
                    <Loader2 className="h-3.5 w-3.5 animate-spin" />
                  )}
                  <span>Save Follow-up</span>
                </button>
              </div>
            </form>
          </div>
        </div>
      )}

      {/* Modal 2: Mark Done & Set Next */}
      {activeDoneFu && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-xs p-4">
          <div className="w-full max-w-sm rounded-xl border border-line bg-card p-5 shadow-xl space-y-4">
            <div className="flex items-center justify-between border-b border-line pb-3">
              <h4 className="text-sm font-bold text-ink flex items-center gap-2">
                <CheckCircle2 className="h-4 w-4 text-emerald-500" />
                Complete Follow-up
              </h4>
              <button
                type="button"
                onClick={() => setActiveDoneFu(null)}
                className="text-ink-subtle hover:text-ink transition-colors p-1"
              >
                <X className="h-4 w-4" />
              </button>
            </div>

            <form onSubmit={handleMarkDoneSubmit} className="space-y-4">
              <div className="space-y-1">
                <label className="text-xs font-medium text-ink-muted">
                  Outcome Note (Optional)
                </label>
                <textarea
                  rows={2}
                  value={outcomeNote}
                  onChange={(e) => setOutcomeNote(e.target.value)}
                  placeholder="e.g. Customer requested quote, wants to visit Saturday..."
                  className="w-full rounded-lg border border-line bg-inset p-3 text-xs text-ink placeholder:text-ink-subtle/60 focus:border-accent focus:outline-none"
                />
              </div>

              {/* Loop (§9): Checkbox to schedule next follow-up immediately */}
              <div className="space-y-2 pt-2 border-t border-line/50">
                <label className="flex items-center gap-2 text-xs font-semibold text-ink cursor-pointer select-none">
                  <input
                    type="checkbox"
                    checked={andScheduleNext}
                    onChange={(e) => setAndScheduleNext(e.target.checked)}
                    className="h-4 w-4 rounded border-line text-accent focus:ring-accent"
                  />
                  <span>Schedule next follow-up now</span>
                </label>

                {andScheduleNext && (
                  <div className="pl-6 pt-1">
                    <DatePicker
                      value={nextDate}
                      onChange={(d) => setNextDate(d)}
                      align="left"
                    />
                  </div>
                )}
              </div>

              <div className="flex items-center justify-end gap-2 pt-2">
                <button
                  type="button"
                  onClick={() => setActiveDoneFu(null)}
                  className="rounded-lg border border-line px-3.5 py-2 text-xs font-medium text-ink hover:bg-hover transition-colors"
                >
                  Cancel
                </button>
                <button
                  type="submit"
                  disabled={updateMutation.isPending}
                  className="flex items-center gap-1.5 rounded-lg bg-emerald-500 px-4 py-2 text-xs font-semibold text-white shadow-xs hover:opacity-90 transition-opacity disabled:opacity-50"
                >
                  {updateMutation.isPending && (
                    <Loader2 className="h-3.5 w-3.5 animate-spin" />
                  )}
                  <span>Complete</span>
                </button>
              </div>
            </form>
          </div>
        </div>
      )}

      {/* Modal 3: Reschedule */}
      {activeRescheduleFu && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-xs p-4">
          <div className="w-full max-w-sm rounded-xl border border-line bg-card p-5 shadow-xl space-y-4">
            <div className="flex items-center justify-between border-b border-line pb-3">
              <h4 className="text-sm font-bold text-ink flex items-center gap-2">
                <RotateCcw className="h-4 w-4 text-accent" />
                Reschedule Follow-up
              </h4>
              <button
                type="button"
                onClick={() => setActiveRescheduleFu(null)}
                className="text-ink-subtle hover:text-ink transition-colors p-1"
              >
                <X className="h-4 w-4" />
              </button>
            </div>

            <form onSubmit={handleRescheduleSubmit} className="space-y-4">
              <div className="space-y-1.5">
                <label className="text-xs font-medium text-ink-muted">
                  New Follow-up Date (IST)
                </label>
                <div>
                  <DatePicker
                    value={rescheduleDate}
                    onChange={(d) => setRescheduleDate(d)}
                    align="left"
                  />
                </div>
              </div>

              <div className="flex items-center justify-end gap-2 pt-2">
                <button
                  type="button"
                  onClick={() => setActiveRescheduleFu(null)}
                  className="rounded-lg border border-line px-3.5 py-2 text-xs font-medium text-ink hover:bg-hover transition-colors"
                >
                  Cancel
                </button>
                <button
                  type="submit"
                  disabled={updateMutation.isPending}
                  className="flex items-center gap-1.5 rounded-lg bg-accent px-4 py-2 text-xs font-semibold text-inverse shadow-xs hover:opacity-90 transition-opacity disabled:opacity-50"
                >
                  {updateMutation.isPending && (
                    <Loader2 className="h-3.5 w-3.5 animate-spin" />
                  )}
                  <span>Save Reschedule</span>
                </button>
              </div>
            </form>
          </div>
        </div>
      )}
    </div>
  );
}
