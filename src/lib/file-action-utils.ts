...
/**
 * Downloads a file through same-origin backend endpoints to eliminate S3 CORS errors.
 * Platform-aware:
...
export async function downloadFile({
  ...
  try {
    let blob: Blob;
    let responseMime = providedMime;

    if (url.startsWith("/api/") || url.includes("/cars/")) {
      const res = await apiClient.get(url, { responseType: "blob" });
      blob = res.data;
      if (typeof res.headers["content-type"] === "string") {
        responseMime = res.headers["content-type"];
      }
    } else {
 ...
 * fetching through backend endpoints to bypass CORS.
 */
